...
function ReactGoogleMaps(props) {
    ...
    const newArr = locsArray.map(loc => {
        ...
        console.log('FROM LOCARRAY', lat, lng)
    })
  ...
  useEffect(() => {
    const doFetch = async () => {
      ...
      const normalizedData = {
        ...
        return {lat: obsv.lat, lng: obsv.lng, locName: obsv.loc, date: obsv.date}
      })
      console.log({observationLocs})
  ...
  return (
    <form onSubmit={handleSubmitLocal}>
      ...
      {Object.values(SortBy).map(sortType => {
        return (<div>
          <label>
            <input type="radio" value={sortType} checked={sortType === sortLocal} onChange={() => setSortLocal(sortType)}/>
            {sortType}
          </label>
        </div>);
      })}
      ...
      {Object.values(Status).map(stat => {
        return (<div>
          <label>
            <input type="checkbox" value={stat} checked={filterLocal.some(item => item === stat)} onChange={() => handleFilterLocalChange(stat)}/>
            {stat}
          </label>
        </div>);
      })}
...
function BirdList(props) {
  ...
  const filterFunc = bird => filterList.some(status => bird.status === status)

  const birdsRefined = data
    .filter(filterFunc)
    .filter(bird => {
        return showFavorites ? favorites.some(favorite => favorite === bird.id) : true
    })
    .sort((bird1, bird2) => {
  ...
  return (
      <div>
          ...
          <RefinementHeader data={data} statuses={statuses} handleSubmit={handleSubmit} sortBy={sortBy} filterList={filterList}/>
          <button onClick={() => setShowFavorites(!showFavorites)}>Toggle Favorites</button>
          {birdsPage.map(bird => (<BirdCard bird={bird} favorites={favorites} setFavorites={setFavorites} />))}
          <PaginationFooter page={page} handleSetPage={(page) => setPage(page)} lastPage={lastPage}/>
      </div>
  )
  ...
  return (
      <div>
          <Link to={{
              ...
              {/* <div>Name:</div> */}
              <div>{name}</div>
              {hasImages && <img src={images[0]} width={4 * IMG_SIZE} height={3 * IMG_SIZE} />}
          </Link>
          <div onClick={handleFavoriteClick} style={{color: isFavorite ? 'red' : 'black'}}>
              {'<3<3<3'}
          </div>
      </div>
  )
...
function NotFound() {
  ...
  return (
    <div>
      NotFound, Check if URL is correct
    </div>
  )
}
  ...
  return (
    <Router>
      <div>
        ...
        <Routes>
          <Route path="/" element={<BirdList data={data} favorites={favorites} setFavorites={setFavorites}/>} />
          <Route path="/birds/:id" element={<BirdDetail data={data} recordings={recordings} setRecordings={setRecordings} />} />
